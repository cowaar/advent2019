...
object startFirstPart extends App{

  def findPath(input1:String):ListBuffer[Coords]={
  val movementFunctions = new MovementFunctions

  val listInput = input1.split(',').toList
  println(listInput(0))

  var totalList = new ListBuffer[Coords]
  var currentCoords = Coords(0,0)
  for(instruction <- listInput){
    println(instruction)
    val (newList:ListBuffer[Coords],newCoords:Coords) = movementFunctions.addVisitedCoords(currentCoords,instruction.toString,totalList)
